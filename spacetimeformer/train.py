...
def create_dset(config):
    ...
    if config.dset == "metr-la" or config.dset == "pems-bay":
        ...
        if config.dset == "asos":
            ...
            data_loader = DataLoader(data_module, batch_size=config.batch_size, num_workers=cpuCount,shuffle=True if config.phase == "train" else False)
...
def main(args):
    ...
    if args.dset == "stocks":
        ...
        train_loader = DataLoader(TimeSeriesDataset(data_folder='spacetimeformer/data/train', context_length=args.context_points, forecast_length=args.target_points), num_workers=cpuCount,batch_size=args.batch_size, shuffle=True)
        test_loader = DataLoader(TimeSeriesDataset(data_folder='spacetimeformer/data/test', context_length=args.context_points, forecast_length=args.target_points), num_workers=cpuCount, batch_size=args.batch_size, shuffle=False)
        oos_loader = DataLoader(TimeSeriesDataset(data_folder='spacetimeformer/data/oos', context_length=args.context_points, forecast_length=args.target_points), num_workers=cpuCount,batch_size=args.batch_size, shuffle=False)
